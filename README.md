...
**Step 3** -- Copy the `ACPI` and `KEXT` files from the older version of OC to the newer version of OC (Notes: JUST COPY what is not on the `kext` folder generated by `OC-Gen-X`!!!)
